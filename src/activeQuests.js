...
// --- DOM Elements ---
const lostQuestsContainer = document.getElementById(
  "lostActiveQuestsContainer"
);
const foundQuestsContainer = document.getElementById(
  "foundActiveQuestsContainer"
);
...
function loadActiveQuests(userId, field, container, label) {
  ...
  container.innerHTML =
    '<p style="text-align: center; color: #555;">Loading quests...</p>';
  ...
  try {
    ...
    onSnapshot(activeQuestsQuery, (snapshot) => {
      ...
      if (posts.length === 0) {
        container.innerHTML = `<p style="text-align: center; color: #555;">No active quests for ${label}.</p>`;
        return;
      }
      ...
      posts.forEach((post) => {
        ...
        if (!isOwner && !isCompleted) {
          ...
          completedButton.textContent = "Complete Quest!";
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user) {
    ...
    if (lostQuestsContainer)
      lostQuestsContainer.innerHTML =
        '<p style="text-align: center; color: #555;">Please log in.</p>';
    if (foundQuestsContainer)
      foundQuestsContainer.innerHTML =
        '<p style="text-align: center; color: #555;">Please log in.</p>';
    return;
  }
